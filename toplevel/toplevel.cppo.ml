...
let setup_toplevel () =
  JsooTop.initialize ();
  Toploop.parse_toplevel_phrase :=  (Reason_util.correctly_catch_parse_errors Reason_toolchain.JS.canonical_toplevel_phrase);
...
let run _ =
  ...
  let history_down e =
    ...
    let pos = (Js.Unsafe.coerce textbox)##selectionStart in
    try
  ...
  let history_up   e =
    let txt = Js.to_string textbox##value in
    let pos = (Js.Unsafe.coerce textbox)##selectionStart - 1  in
    try
  ...
  begin (* setup handlers *)
    ...
    textbox##onkeydown <- Dom_html.handler (fun e ->
        ...
        | 13 when not (meta e) && not (shift e)  -> Lwt.async execute; Js._false
        | 13 when (shift e) || (meta e) -> Lwt.async (resize ~container ~textbox); Js._true
        | 09 -> Indent.textarea (Js.Unsafe.coerce textbox); Js._false
